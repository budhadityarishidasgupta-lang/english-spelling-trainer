...
def render_practice_mode(*, show_back_button: bool = True):
    """
    Practice Mode UI to be mounted inside the main Maths shell.
    IMPORTANT:
      - No st.set_page_config() here (shell owns it)
      - Session state keys are namespaced to avoid collisions with test mode
    """
    ...
    if show_back_button:
        st.markdown("---")
        if st.button("⬅ Back to Home", use_container_width=True, key="practice_back_home"):
            st.session_state.mode = "home"
            st.rerun()
